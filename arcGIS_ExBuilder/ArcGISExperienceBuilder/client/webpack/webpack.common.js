exports.getCommon = function (commonOptions = {}) {
  ...
  const jsApiUrl = process.env.ARCGIS_JSAPI_URL || 'https://js.arcgis.com/4.30/';
      ...
      if(process.env.TSCHECK && fs.existsSync(path.join(__dirname, `../tsconfig/tsconfig-${chunkName}.json`))){
        plugins = plugins.concat([
          new ForkTsCheckerWebpackPlugin({
            async: true,
            typescript: {
              // Memory limit for the checker process in MB. The default value is 2048 MB. If the process exits with the allocation failed error, try to increase this number.
              memoryLimit: 1024 * 4,
