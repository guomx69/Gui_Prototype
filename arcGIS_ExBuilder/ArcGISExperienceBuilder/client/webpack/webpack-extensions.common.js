...
const { optimization } = require('./webpack.common');
...
function getWidgetsInfoForWebpack(widgetsFolder){
  ...
  webpackCommon.visitFolder(widgetsFolder, (widgetFolder, widgetName) => {
    ...
    if(infos.find(info => info.name === widgetName)){
      console.error('Name is duplicated.', widgetName);
      return;
    }
  ...
  if(fs.existsSync(sharedCodeFolder)){
    ...
    fs.readdirSync(sharedCodeFolder).forEach(sharedCodeFile => {
      if (!fs.statSync(path.join(sharedCodeFolder, sharedCodeFile)).isDirectory()) {
        ...
        entries[entryName] = [publicPathFile, path.join(sharedCodeFolder, sharedCodeFile)]
      } else {
        if (sharedCodeFile === 'translations') {
          ...
          if (files.find(f => f.from === path.join(sharedCodeFolder, sharedCodeFile))) {
          ...
          files.push({ from: path.join(sharedCodeFolder, sharedCodeFile), to: 'widgets/shared-code/translations'})
          ...
          fs.readdirSync(path.join(sharedCodeFolder, 'lib')).forEach(libSubFolder => {
            ...
            if (fs.existsSync(subTranslationFolder) && fs.statSync(subTranslationFolder).isDirectory()) {
              ...
              if (files.find(f => f.from === subTranslationFolder)) {
              ...
              files.push({ from: subTranslationFolder, to: `widgets/shared-code/lib/${libSubFolder}/translations`})
            }
            ...
            if (fs.existsSync(subAssets) && fs.statSync(subAssets).isDirectory()) {
              ...
              if (files.find(f => f.from === subAssets)) {
              ...
              files.push({ from: subAssets, to: `widgets/shared-code/lib/${libSubFolder}/assets`})
            }
          })
          ...
          files.push({ from: path.join(sharedCodeFolder, sharedCodeFile), to: 'widgets/shared-code/assets'})
        }
      }
    })
  }
